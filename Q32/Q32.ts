...
let current_users:string[]=["user1" ,"user2","user3" ,"user4","user5", "Shams", "Abroo" ];
let new_users=["Aroba", "Arzo", "Shams", "Bazila", "Abroo"];

for(let new_users1 of new_users){ const lowercase_new_users=new_users1.toLocaleLowerCase()
  let a=  current_users.map(user=> user.toLocaleLowerCase()).includes(lowercase_new_users);    
    ...
    else{console.log(`the username ${new_users1} is available.`)}}